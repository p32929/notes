...
export async function saveData() {
  try {
    ...
    // Check if settings changed (including Split View settings)
    const settingsChanged =
      ...
      states.activeEditorId !== previousStates.activeEditorId ||
      JSON.stringify(states.layout) !== JSON.stringify(previousStates.layout)
...
async function saveSettings(states: any) {
  const settings = {
    ...
    activeEditorId: states.activeEditorId
  }
...
export async function getData() {
  try {
    ...
    const stateData = {
      ...
      // Split View settings with defaults
      isSplitView: settings?.isSplitView || false,
      layout: settings?.layout || createDefaultLayout(),
      activeEditorId: settings?.activeEditorId || ''
    }
